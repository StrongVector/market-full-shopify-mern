...
const RegisterForm = () => {
  ...
  const handlesubmit = (e) => {
    e.preventDefault();
    setLable("you will reDirected to Login page ");
  };
  return (
    <form
      onSubmit={handlesubmit}
      className="container mx-auto flex flex-col items-center bg-white w-fit justify-center gap-3 p-4
   ...
   shadow-sm border-slate-300 placeholder-slate-300 focus:outline-none text-slate-800
    focus:border-sky-500 focus:ring-sky-500   rounded-md flex justify-start
     sm:text-sm focus:ring-1"
      ...
      <button
        type="submit"
        className="bg-blue-600 w-72 cursor-pointer py-1 rounded-lg text-white"
      >
        Sign Up
      </button>
      <p
